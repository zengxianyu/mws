...
#optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, net.encoder.pred.parameters()), lr=1e-4)
...
def tokentensor2text(tokens):
    text = id2w[tokens.cpu().numpy()]
    list_t = [' '.join(t) for t in text]
    return list_t

def visualize(img, cappred, captions, big_msk, loss, i):
    writer.add_single_image("image", torchvision.utils.make_grid(img*std+mean), i)
...
#value = np.sqrt((yy.astype(np.float)-msk_size/2)**2 + (xx.astype(np.float)-msk_size/2)**2)
#value = np.exp(0.4*value)
#value = torch.Tensor(value).cuda()
#net.load_state_dict(torch.load("./output/debug/checkpoints/28.pth"))
...
def train():
    ...
    for num in range(args.numstart+1, args.numtrain):
        ...
        if num % args.numprint == 0:
            ...
            visualize(img[:4], cappred[:4],
                    captions[:4], big_msk[:4], loss.item(), num)
            writer.write_html()
