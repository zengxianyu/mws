...
from networks.selftrainloss import CoTrainLoss
...
parser.add_argument('--wr', default=1e-3, type=float)
parser.add_argument('--wt', default=1e-2, type=float)
parser.add_argument('--wc', default=1e-1, type=float)
...
cap_train_loader = torch.utils.data.DataLoader(
    ...
    batch_size=args.batchsize, shuffle=False, num_workers=args.numworkers, pin_memory=True, collate_fn=caption_collate_fn)
...
ttloss = TransLoss()
coloss = CoTrainLoss(vmean=mean, vstd=std)
...
def train():
    ...
    for num in range(args.numstart+1, args.numtrain):
        ...
        netp.rcap = False
        big_msk_c = netc(img)
        big_msk_c = F.sigmoid(big_msk_c)
        big_msk_p = netp(img)
        big_msk_p = F.sigmoid(big_msk_p)
        _l, _g = coloss(img, [big_msk_p, big_msk_c])
        loss = _l * args.wc
        loss.backward()
        #if num % args.numprint == 0:
        #    writer.add_single_image("image", torchvision.utils.make_grid(img[:n_vis]), i)
        #    big_msk = torch.cat([big_msk[:n_vis], big_msk, big_msk], 1)
        #    writer.add_single_image("mask", torchvision.utils.make_grid(big_msk), i)

        optimizer.step()
        if num % args.numprint == 0:
            _g = _g[:n_vis]
            _g = torch.cat((_g, _g, _g), 1)
            _p = (big_msk_p[:n_vis] + big_msk_c[:n_vis])/2
            _p = torch.cat((_p, _p, _p), 1)
            writer.add_single_image("_p", torchvision.utils.make_grid(_p), num)
            writer.add_single_image("_g", torchvision.utils.make_grid(_g), num)
            img = img*std+mean
            writer.add_single_image("img", torchvision.utils.make_grid(img), num)
            writer.write_html()
            print(f"train iteration {num} | img loss {loss.item()}")
